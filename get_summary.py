...
def get_info(df, query, user_config, pred_metrics, pred_dist, quantiles=[0.10, 0.25, 0.5, 0.75, 0.90], filtered=False):
    ...
    if "cost" in user_config and user_config["cost"] is not None:
        ...
        msg += f"""We also calculate a proxy for the expected profit: (unit_price x quantity - cost) x (1 - q), where q is the quantile of a given unit price. Based on this, the optimal unit price is {expected_profit["optimal_price"]} with maximal expected profit {expected_profit["max_expected_profit"]}. The unit price range so that the expected profit is >= 90% of the maximal expected profit is {expected_profit["price_range_ge_90pct"]}. The unit price range so that the expected profit is >= 50% of the maximal expected profit is {expected_profit["price_range_ge_50pct"]}.
        """
    return msg
